...
public abstract class AbstractGraph<T> {
    ...
    protected List<Vertex<T>> vertices;                   // Düğümleri tutan liste
  
    protected Map<T, List<Edge<T>>> edges;                // Değere göre düğümün kenar listesinin saklandığı map

    protected Map<T, Vertex<T>> verticesMap;              // Düğümlere kolay erişmek için kullanılan map <değer, düğüm>
  
    boolean directed=true;                               // Çizge yönlü mü
    ...
    public String toString() {
        ...
        for (Map.Entry<T,List<Edge<T>>> entry:edges.entrySet())
            for (Edge<T> edge:entry.getValue())
                sb.append(edge.from.value+(edge.weight!=0?" - "+edge.weight:"")+" -> "+edge.to.value+"\n");
        return sb.toString();
    }
